import { normalizeOrgSize, ORG_SIZE_OPTIONS, WishKey } from "./data";
...
export default function Step5Wishes({
  ...
  const hasValidOrgSize = normalizeOrgSize(orgSize) !== "";

  return (
    <section>
      <p className="mb-2 text-[11px] uppercase tracking-[0.6px] text-[#aaa]">What would genuinely help</p>
      <h2 className="mb-2 text-[30px] leading-tight font-bold">
        If your employer wanted to make your life outside work a little easier - what would actually matter to you?
      </h2>
        ...
        {wishes.map((w) => {
          ...
          return (
            <button
              key={w.k}
              type="button"
              onClick={() => onToggleWish(w.k)}
              className={`flex cursor-pointer items-center gap-3 rounded-[14px] border-2 p-3 text-left transition ${
                selected ? "border-[#1D9E75] bg-[#f0fdf9]" : "border-[#e8e8e8] bg-white hover:border-[#bbb] hover:bg-[#fafafa]"
              }`}
            >
              ...
              <span
                className={`flex h-4 w-4 items-center justify-center rounded-[4px] border-2 text-[9px] ${
                  selected ? "border-[#1D9E75] bg-[#1D9E75] text-white" : "border-[#ddd]"
                }`}
              >
                {selected ? "✓" : ""}
              </span>
      ...
      <div className="mt-6 rounded-[14px] border border-[#e8e8e8] bg-[#fafafa] p-4">
        <p className="mb-1 text-[11px] uppercase tracking-[0.6px] text-[#aaa]">Tell us about yourself</p>
        <p className="mb-4 text-[12px] text-[#888]">Organization name is optional. Organization size is required to submit your survey.</p>
        <div className="grid gap-3">
          <div>
            <label htmlFor="survey-org-name" className="mb-1 block text-[12px] font-semibold text-[#555]">
          ...
          <div>
            <label htmlFor="survey-org-size" className="mb-1 block text-[12px] font-semibold text-[#555]">
              Organization size *
            </label>
            <select
              id="survey-org-size"
      ...
      <div className="mt-6 flex items-center justify-between">
        <button type="button" onClick={onBack} className="text-[13px] text-[#aaa] hover:text-[#333]">
          Back
        </button>
        <button
          type="button"
          disabled={selWishes.length === 0 || !hasValidOrgSize}
          onClick={onSubmit}
          className="rounded-[10px] bg-[#1D9E75] px-7 py-2.5 text-[14px] font-bold text-white disabled:cursor-not-allowed disabled:bg-[#ddd] disabled:text-[#aaa]"
        >
          See my snapshot
        </button>
      </div>
